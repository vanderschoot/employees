...
public class Roles extends CustomComponent {
	...
	private Window win = new Window("role");
	...
	public Roles(DataModel dataModel, Navigator navigator) {
	...
	private void createTable(DataModel dataModel, final Navigator navigator) {	
		...
		roleContainer.removeAllContainerFilters();
        ...
        table.setVisibleColumns(DataModel.ROLE_COLS);
        table.setColumnHeaders(DataModel.ROLE_LABELS);
       
        /* bind form to clicked table item, and set fields in the right order */
        final List<Object> orderedProperties = Arrays.asList(DataModel.ROLE_COLS);
	...
	private void createTableControls() {
		...
		btnAdd.addListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
		        ...
		        tempItemId = roleContainer.addItem();
		        Item tempItem = roleContainer.getItem(tempItemId);
		        if (tempItem != null) {
		            List<Object> orderedProperties = Arrays.asList(DataModel.ROLE_COLS);
				...
				navigator.addDialog(win, 400, 150);
			}
		});
		...
		btnDel.addListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
		    	try {
		    		System.out.println("Roles, btnDel, table.getValue() = " + table.getValue());
		    		Object itemId = table.getValue();
		    		ReturnStatus sts = dataModel.deleteRole(itemId);
		    		if (sts.isSucces()) {
		    			if (roleContainer.removeItem(itemId)) {
		    				navigator.showNotification("Role has been removed");
		    			} else {		    				
	...
	private Component addSearchFilter() {
		...
		int i = 0;
		for (final Object pn : DataModel.ROLE_COLS) {
			final TextField sf = new TextField();
			searchfields.addComponent(sf);
			sf.setWidth("100%"); 
			sf.setInputPrompt(DataModel.ROLE_LABELS[i]); 
			sf.setImmediate(true); 
			searchfields.setExpandRatio(sf, 1);
			sf.addListener(new Property.ValueChangeListener() {
				public void valueChange(ValueChangeEvent event) {
					roleContainer.removeContainerFilters(pn);
					if (sf.toString().length() > 0 && !pn.equals(sf.toString())) {
						roleContainer.addContainerFilter(pn, sf.toString(), true, false); 
					} 
					navigator.showNotification( "" + roleContainer.size() + " matches found");
				} 
			}); 
			i++;
		} 
	...
	private void createForm() {
        ...
        form.setFormFieldFactory(new DefaultFieldFactory() {
            ...
            public Field createField(Item item, Object propertyId,Component uiContext) {
                ...
                for (int i = 0; i < DataModel.ROLE_COLS.length; i++) {
                    if (DataModel.ROLE_COLS[i].equals(propertyId)) {
                        field.setCaption(DataModel.ROLE_LABELS[i]);
                    }
                }
                return field;
            }
                ...
                if (form.isValid()) {
                	...
                	try {
                		if (tableAction.equals("add")) {
                			ReturnStatus sts = dataModel.addRole(form.getField("name").getValue().toString());
                			if (sts.getMessage() != null) {
                				Item item = roleContainer.getItem(tempItemId);
                				item.getItemProperty("ID").setValue(sts.getMessage());
                			}
                			//dataModel.refreshRoles();
                			//roleContainer = dataModel.getRoleContainer();
                			//table.setContainerDataSource(roleContainer);
                		}
                		if (tableAction.equals("edit")) {
                			dataModel.updateRole(table.getValue(),
        ...
        btnCancel.addListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (tableAction.equals("add")) {
	               	try {
	               		roleContainer.removeItem(tempItemId);
					} catch (UnsupportedOperationException e) {
						e.printStackTrace();
					}
				}
