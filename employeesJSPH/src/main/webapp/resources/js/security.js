...
function registerREST() {
	...
	if (messages != "") {
		...
		var queryparams = 	"&userName=" + $('#inp2-username').val() + 
		  ...
		  $.ajax({	type: "GET",
              url: serverurl + '/SecurityController?action=register' + queryparams,
              dataType: "json",
...
function askpasswordREST() {
	...
	if (messages != "") {
		...
		$.ajax({	type: "GET",
		url: serverurl + '/SecurityController?action=askpassword' + queryparams,
		dataType: "json",
		beforeSend : function(req) {
		req.withCredentials = true;
		req.setRequestHeader('Authorization', auth);
...
function loginREST() {
	...
	if (messages != "") {
		...
		$.ajax({type: "GET",
	            url: serverurl + '/SecurityController?action=login',
	            dataType: "json",
	      	    beforeSend : function(req) {
	      	    	req.withCredentials = true;
	      	        req.setRequestHeader('Authorization', auth);
...
function logoutREST() {
	  var serverurl = getServerUrl();
	  $.ajax({	type: "GET",
	              url: serverurl + '/SecurityController?action=logout',
	              dataType: "json",
	      	      beforeSend : function(req) {
	        	    	req.withCredentials = true;
...
function checkroleREST(user, role) {
	  ...
	  $.ajax({	type: "GET",
		  		async:false,
	            url: serverurl + '/SecurityController?action=checkrole&user=' + user + '&role=' + role,
	            dataType: "json",
	      	    beforeSend : function(req) {
	        	  	req.withCredentials = true;
	        	  	req.setRequestHeader('Authorization', auth);
...
function loggedinREST(user) {
	  ...
	  if (user) {
		  $.ajax({	type: "GET",
			  		async:false,
		            url: serverurl + '/SecurityController?action=loggedin&userName=' + user,
		            dataType: "json",
		      	    beforeSend : function(req) {
		        	  	req.withCredentials = true;
		        	  	req.setRequestHeader('Authorization', auth);
