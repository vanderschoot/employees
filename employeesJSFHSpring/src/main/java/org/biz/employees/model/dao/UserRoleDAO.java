...
public class UserRoleDAO extends AbstractDAO<UserRole> {
    ...
    public ReturnStatus createroles(int userId, int[] roleList) {
    	System.out.println("createroles : userId = " + userId);
    	UserRole urole;
        ...
        if (usr != null) {
        	for (int i=0; i < roleList.length; i++) {        		
                ...
                Role rol = em.find(Role.class, roleList[i]);
                if (rol != null) {
                	System.out.println("create role id= " + rol.getRoleId() + " / name = " + rol.getName());
                	urole.setRole(rol);
                    em.persist(urole);
                }                
        	}	        
