...
public class SecurityService  {
    	...
    	if (result[0].equals("OK")) {
        	...
        	if (checkPassword(result[1], result[2])) {
				try {
					...
					session.setAttribute("user", req.getRemoteUser());
			    ...
			    System.out.println("login, RequestedSessionId = " + req.getRequestedSessionId());
    ...
    public ReturnStatus logout(HttpServletRequest req, HttpSession session) {
		...
		try {
	        ...
	        System.out.println("uitloggen niet gelukt");
			return new ReturnStatus(false,"uitloggen niet gelukt");
		}      
    }
    
    public LoginStatus loggedin(String user, HttpSession session) {
        System.out.println("loggedin");
        
        String currentuser = (String) session.getAttribute("user");
        System.out.println("user = " + user +  " /  currentuser = " + currentuser);

        if (currentuser == null) {
            System.out.println("currentuser is NULL, nobody is loggged in");
        	return new LoginStatus(false, null, null, "Nobody is logged in");     
        } else if (currentuser.equals(user)) {
            System.out.println("currentuser is loggedin");
	        return new LoginStatus(true, currentuser, getroles(user), "currentuser is loggedin");
        } else {
        	return new LoginStatus(true, currentuser, getroles(user), "other user is loggedin");
        }
    }
